...
Examples:
  ...
  python cleanup_directories.py --verbose
    ...
    try:
        ...
        needs_cleanup = any(
            info['usage_percent'] > 100 for info in status.values()
        )
        ...
        if args.dry_run:
            ...
            for dir_name, info in status.items():
                if info['usage_percent'] > 100:
                    print(f"\n{dir_name}:")
                    files = cleanup_manager.get_file_info(cleanup_manager.generated_images_dir if dir_name == 'generated_images' else cleanup_manager.uploads_dir)
                    ...
                    for file_path, file_size, mtime in files:
                        if removed_size >= to_remove_size:
                            break
                        print(f"  Would delete: {file_path.name} ({file_size / (1024**2):.1f}MB)")
