    ...
    def _check_lora_compatibility(self, lora_path: str) -> bool:
        ...
        try:
            ...
            if not lora_path.endswith(('.safetensors', '.bin', '.pt', '.pth')):
                logger.error(f"   - Unsupported file format: {os.path.splitext(lora_path)[1]}")
                return False
            ...
            try:
                ...
                if not isinstance(lora_weights, dict) or not lora_weights:
                    logger.error(f"   - LoRA file does not contain valid weight dictionary")
                    return False
                ...
                if not has_compatible_keys:
                    logger.error(f"   - LoRA file does not appear to be compatible with FLUX models")
                    return False
                ...
                logger.error(f"   - Failed to load LoRA file for compatibility check: {str(load_error)}")
                return False
