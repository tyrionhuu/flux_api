...
# FLUX API Docker Compose Runner
# Simplified script that handles port and token configuration
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --backend-only)
            ...
            ;;
        -h|--help)
            echo "FLUX API Docker Compose Runner"
            echo "==============================="
            echo ""
            ...
            echo "  --backend-only        Run in backend-only mode (disable frontend)"
            ...
            echo "  $0 --backend-only -t hf_your_token_here    # Backend-only mode"
            echo ""
...
echo "🚀 Starting FLUX API with:"
echo "   Port: $FP4_API_PORT"
echo "   Token: ${HUGGINGFACE_HUB_TOKEN:0:10}..."
echo "   Build: ${BUILD_FLAG:-"no"}"
echo "   Frontend: $([ "$BACKEND_ONLY" = true ] && echo "disabled" || echo "enabled")"
echo ""
...
if [ "$BACKEND_ONLY" = true ]; then
    ...
    echo "📱 Access the UI at: http://localhost:$FP4_API_PORT/ui"
fi
