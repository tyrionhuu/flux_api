    ...
    for attempt in range(max_retries):
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                print(f"   Found {len(pids)} process(es) using port {port} (attempt {attempt + 1}/{max_retries})")
                    ...
                    if pid.strip():
                        try:
                            ...
                            try:
                                ...
                                print(f"   ✅ Process {pid_int} terminated successfully")
                ...
                if result.returncode == 0:
                    ...
                    for line in lines:
                        if f":{port}" in line and "LISTEN" in line:
                            print(f"   🚫 Found process using port {port}, attempting to kill...")
