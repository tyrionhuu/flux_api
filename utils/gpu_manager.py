...
class GPUManager:
    ...
    def _initialize_gpu_status(self):
        ...
        for gpu_id in range(self.device_count):
            try:
                ...
                logger.info(f"Initialized GPU {gpu_id}: {total_memory_gb:.1f}GB total, {free_memory_gb:.1f}GB free")
    ...
    def select_best_gpu(self) -> Optional[int]:
        ...
        for i in range(self.device_count):
            ...
            try:
                ...
                result = subprocess.run(
                    ["nvidia-smi", "--query-gpu=memory.used,memory.total", "--format=csv,noheader,nounits", "-i", str(i)],
                ...
                if result.returncode == 0:
                    ...
                    if lines and ',' in lines[0]:
                        ...
                        total_memory_smi = float(total_memory_smi) / 1024  # Convert MB to GB
        ...
        sufficient_memory_gpus = [(i, free, total) for i, free, total in gpu_memory if free >= 8.0]
        ...
        if not sufficient_memory_gpus:
            ...
            print(f"⚠️  Selecting GPU {best_gpu[0]} with only {best_gpu[1]:.1f}GB free (may fail)")
    ...
    def check_gpu_memory_sufficient(self, gpu_id: int, required_memory_gb: float = 16.0) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if lines and ',' in lines[0]:
                    ...
                    print(f"   📊 GPU {gpu_id}: {used_memory:.1f}GB used, {total_memory:.1f}GB total, {free_memory:.1f}GB free")
                    print(f"   {'✅' if free_memory >= required_memory_gb else '❌'} GPU {gpu_id}: {'Sufficient' if free_memory >= required_memory_gb else 'Insufficient'} memory ({free_memory:.1f}GB free, {required_memory_gb}GB required)")
            ...
            print(f"   📊 GPU {gpu_id}: {allocated_memory:.1f}GB allocated, {total_memory:.1f}GB total, {free_memory:.1f}GB free (PyTorch)")
            print(f"   {'✅' if free_memory >= required_memory_gb else '❌'} GPU {gpu_id}: {'Sufficient' if free_memory >= required_memory_gb else 'Insufficient'} memory ({free_memory:.1f}GB free, {required_memory_gb}GB required)")
    ...
    def find_available_gpu_for_model(self, required_memory_gb: float = 16.0) -> Optional[int]:
        ...
        if best_gpu is not None:
            print(f"✅ Found available GPU {best_gpu} with {best_free_memory:.1f}GB free memory")
        ...
        for gpu_id in range(self.device_count):
            try:
                ...
                if result.returncode == 0:
                    lines = result.stdout.strip().split('\n')
                    if lines and ',' in lines[0]:
                        used_memory, total_memory, temperature, utilization = lines[0].split(',')
        ...
        with self.assignment_lock:
            ...
            for gpu_id, status in self.gpu_status.items():
                ...
                if (memory_usage > self.memory_threshold or 
                    status.temperature > self.temperature_threshold or
                    status.utilization > self.utilization_threshold):
                    overloaded_gpus.append(gpu_id)
                elif (memory_usage < 0.3 and  # Less than 30% usage
                      status.temperature < 60.0 and  # Cool temperature
                      status.utilization < 0.5):  # Low utilization
    ...
    def _perform_rebalancing(self, overloaded_gpus: List[int], underutilized_gpus: List[int]):
        """Move services from overloaded GPUs to underutilized ones"""
        logger.info(f"🔄 Rebalancing: Moving services from {overloaded_gpus} to {underutilized_gpus}")
        ...
        for overloaded_gpu in overloaded_gpus:
            ...
            for service_name in services_to_move:
                ...
                best_gpu = self._find_best_gpu_for_service(service, exclude_gpus=[overloaded_gpu])
                
                if best_gpu is not None and best_gpu in underutilized_gpus:
                    # Migrate service
                    if self._migrate_service(service_name, best_gpu):
                        logger.info(f"✅ Migrated {service_name} from GPU {overloaded_gpu} to GPU {best_gpu}")
                        break
    ...
    def _handle_failures(self):
        ...
        for gpu_id, status in self.gpu_status.items():
            ...
            if current_time - status.last_update > 30:
                logger.warning(f"⚠️  GPU {gpu_id} appears unresponsive, migrating services")
    ...
    def _handle_gpu_failure(self, failed_gpu: int):
        ...
        for service_name in services_to_migrate:
            ...
            alternative_gpu = self._find_best_gpu_for_service(
                ...
                exclude_gpus=[failed_gpu]
            )
            
            if alternative_gpu is not None:
                if self._migrate_service(service_name, alternative_gpu):
                    logger.info(f"✅ Migrated {service_name} from failed GPU {failed_gpu} to GPU {alternative_gpu}")
            else:
                logger.error(f"❌ No alternative GPU available for {service_name}")
    
    def _find_best_gpu_for_service(self, service: ServiceInfo, exclude_gpus: Optional[List[int]] = None) -> Optional[int]:
        ...
        for gpu_id, status in self.gpu_status.items():
            ...
            free_gb = realtime_free_gb if realtime_free_gb is not None else status.free_memory_gb
            total_gb = realtime_total_gb if realtime_total_gb is not None else status.total_memory_gb
            ...
            temperature_score = 1.0 - (status.temperature / 100.0)  # Lower temperature = higher score
            utilization_score = 1.0 - status.utilization  # Lower utilization = higher score
            
            # Weighted score
            score = (memory_score * 0.5 + temperature_score * 0.3 + utilization_score * 0.2)
    ...
    def _migrate_service(self, service_name: str, target_gpu: int) -> bool:
        ...
        logger.info(f"🔄 Migrated {service_name} from GPU {old_gpu} to GPU {target_gpu}")
        return True

    # ===== SERVICE REGISTRATION AND MANAGEMENT =====

    def register_service(self, service_name: str, required_memory_gb: float, priority: int = 1) -> Optional[int]:
        """Register a new service and assign it to the best available GPU"""
        with self.assignment_lock:
            # Check if service already exists
            if service_name in self.services:
                logger.info(f"✅ Service {service_name} already registered on GPU {self.services[service_name].current_gpu}")
            ...
            if best_gpu is None:
                logger.warning(f"⚠️  Dynamic GPU selection failed for {service_name}, trying fallback method")
    ...
    def print_status_summary(self):
        ...
        print(f"📊 Monitoring: {'🟢 Active' if summary['overall']['monitoring_active'] else '🔴 Inactive'}")
        ...
        for service_name, info in summary["services"].items():
            print(f"✅ {service_name}: GPU {info['gpu']} (Memory: {info['memory_required']}GB, Priority: {info['priority']})")
        ...
        for gpu_id, info in summary["gpus"].items():
            ...
            print(f"{status_icon} GPU {gpu_id}: {memory_pct:.1f}% used, {info['temperature']:.1f}°C, {info['utilization']*100:.1f}% util")
            if info["services"]:
...
def register_service_dynamically(service_name: str, required_memory_gb: float, priority: int = 1) -> Optional[int]:
...
def set_cuda_visible_devices_for_service(service_name: str, required_memory_gb: float) -> Optional[str]:
    ...
    logger.info(f"🔧 Set CUDA_VISIBLE_DEVICES={cuda_devices} for {service_name} (GPU {gpu_id})")
