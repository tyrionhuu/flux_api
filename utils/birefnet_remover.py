...
class BiRefNetRemover:
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"BiRefNet model loaded successfully, time: {load_duration:.2f}s"
            )
            return True
    ...
    def remove_background(
        self, image: Image.Image, bg_strength: Optional[float] = None
    ) -> Image.Image:
        """
        ...
        try:
            ...
            # Preprocess image
            input_images = (
                self.transform_image(image).unsqueeze(0).to(self.device).half()
            )
...
def remove_background_birefnet(
    image: Image.Image, bg_strength: Optional[float] = None
) -> Image.Image:
    """
