...
class DirectoryCleanup:
    ...
    def __init__(self, generated_images_dir: str = "generated_images", 
        ...
        self.generated_images_limit_bytes = int(generated_images_limit_gb * 1024 * 1024 * 1024)
        ...
        logger.info(f"  Generated images: {self.generated_images_dir} (limit: {generated_images_limit_gb}GB)")
    ...
    def cleanup_directory(self, directory: Path, size_limit_bytes: int, 
                         directory_name: str) -> int:
        """
        ...
        if current_size <= size_limit_bytes:
            logger.debug(f"{directory_name} size ({current_size / (1024**3):.2f}GB) is under limit ({size_limit_bytes / (1024**3):.2f}GB)")
            return 0
        
        logger.info(f"{directory_name} size ({current_size / (1024**3):.2f}GB) exceeds limit ({size_limit_bytes / (1024**3):.2f}GB), cleaning up...")
        ...
        for file_path, file_size, _ in files:
            ...
            try:
                ...
                files_removed += 1
                logger.info(f"Removed old file: {file_path.name} ({file_size / (1024**2):.1f}MB)")
            except OSError as e:
                logger.error(f"Failed to remove file {file_path}: {e}")
        
        if files_removed > 0:
            final_size = self.get_directory_size(directory)
            logger.info(f"{directory_name} cleanup complete: removed {files_removed} files, "
    ...
    def get_status(self) -> dict:
        ...
        return {
            'generated_images': {
                ...
                'usage_percent': (generated_size / self.generated_images_limit_bytes) * 100
            },
            'uploads': {
...
if __name__ == "__main__":
    ...
    for dir_name, info in status.items():
        print(f"  {dir_name}: {info['current_size_gb']:.2f}GB / {info['limit_gb']:.2f}GB ({info['usage_percent']:.1f}%)")
